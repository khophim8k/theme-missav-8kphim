...
@section('content')
    ...
    <div class="flex justify-between mt-1">
        <h1>
            <span class="text-base xl:text-xl text-white block font-bold">
                    {{ $currentMovie->name }}
            </span>
        </h1>
    ...
    <div class="flex flex-wrap justify-center gap-1 py-3 text-white" id="stream-servers">
        @foreach ($currentMovie->episodes->where('slug', $episode->slug)->where('server', $episode->server) as $server)
            <a onclick="chooseStreamingServer(this)" data-type="{{ $server->type }}" data-id="{{ $server->id }}"
                ...
                class="streaming-server hover:cursor-pointer uppercase current bg-[#151111] hover:bg-red-600 hover:bg-opacity-80 shadow-md px-2 py-1 mr-1 rounded text-sm">Server #{{ $loop->index }}
            </a>
        @endforeach
        ...
        <div class="flex flex-col my-3 mt-6 p-2 bg-[#272727] bg-opacity-50">
            <h2 id="heading-{{ $loop->index }}">
                <button class="flex justify-between w-full py-2 font-medium text-left text-slate-200 rounded-sm"
                    ...
                    <span class="text-red-600">{{ $server }}</span>
                </button>
            </h2>
    ...
    <div class="flex rounded-lg p-0 md:p-2 text-[#bbb] bg-[#1511116d] mb-2">
        <div class="w-full pr-2">
            <article class="w-auto rounded-lg text-[#bbb] p-1 mt-1 relative">
                <div id="content" class="overflow-hidden">
                    @if ($currentMovie->content)
                        <span class="short-content"></span>
                        <span class="full-content hidden">{!! $currentMovie->content !!}</span>
                        <button id="toggle-button" class="ml-1 text-nord13">Xem thêm</button>
                    @else
                        <p>Đang cập nhật ...</p>
                    @endif
                </div>
    ...
    <div class="mt-1.5">{!! $currentMovie->tags->map(function ($tag) {
            return '<a class="bg-[#111] text-nord13 hover:text-white px-2 border-[#333] border-[1px]" href="' .
                $tag->getUrl() .
                '" title="' .
                $tag->name .
                '">' .
    ...
    <div class="mt-2.5 p-3 bg-[#1511116d] mb-3 rounded-lg">
        <h3 class="text-sm font-bold text-primary uppercase mt-1.5 mb-3">
            Phim sex đề xuất
        </h3>
...
@section('footer')
    ...
    <script src="/themes/missav/player/js/p2p-media-loader-core.min.js"></script>
    <script src="/themes/missav/player/js/p2p-media-loader-hlsjs.min.js"></script>
    ...
    {{-- Star Rating Plugins --}}
    <script src="/themes/missav/js/plugins/jquery-raty/jquery.raty.js"></script>
    <link href="/themes/missav/js/plugins/jquery-raty/jquery.raty.css" rel="stylesheet" type="text/css" />
    ...
    <script>
        ...
        $('#movies-rating-star').raty({
            ...
            starOff: '/themes/missav/js/plugins/jquery-raty/images/star-off.png',
            starOn: '/themes/missav/js/plugins/jquery-raty/images/star-on.png',
            starHalf: '/themes/missav/js/plugins/jquery-raty/images/star-half.png',
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            const wordLimit = 50; // Số lượng từ tối đa hiển thị
            const contentElement = document.querySelector(".full-content");
            const shortContentElement = document.querySelector(".short-content");
            ...
            if (words.length > wordLimit) {
                shortContentElement.innerText = words.slice(0, wordLimit).join(" ") + "...";
                shortContentElement.classList.remove("hidden");
            ...
            toggleButton.addEventListener("click", function () {
                if (contentElement.classList.contains("hidden")) {
                    ...
                    shortContentElement.classList.add("hidden");
            ...
            if (type == 'embed') {
                if (vastAds) {
                    ...
                    const objSetupFake = {
                        ...
                        aspectratio: "16:9",
                        width: "100%",
                        file: "/themes/missav/player/1s_blank.mp4",
                        volume: 100,
                        mute: false,
                        autostart: true,
                        advertising: {
